...
public class BypassMutation extends Mutation {
    ...
    public void onAssign(Player player) {
        ItemStack a1 = AbilityUtils.createAbilityItem(Material.IRON_SWORD,
                "§7[A1] Rapier State", "Ignore 50% armor 7s | 30s CD");
        ItemStack a2 = AbilityUtils.createAbilityItem(Material.BARRIER,
                "§7[A2] Mutation Lock", "Disable target's mutation 10s | 45s CD");
        ItemStack a3 = AbilityUtils.createAbilityItem(Material.ENDER_EYE,
                "§7[A3] Phantom Dash", "Dash 7 blocks, 5 heart dmg | 16s CD");
    ...
    public void activateAbility3(Player player) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            for (LivingEntity e : hit) {
                ...
                world.spawnParticle(Particle.EXPLOSION, e.getLocation(), 3, 0.2, 0.2, 0.2, 0);
