...
public class HellfireMutation extends Mutation {
    ...
    public void onAssign(Player player) {
        ItemStack a1 = AbilityUtils.createAbilityItem(Material.BLAZE_ROD,
                "§c[A1] Hellfire Rush", "Zigzag dash, 4 dmg + fire trail | 16s CD");
        ItemStack a2 = AbilityUtils.createAbilityItem(Material.FIRE_CHARGE,
                "§c[A2] Flame Whip", "3-hit fire combo | 15s CD");
        ItemStack a3 = AbilityUtils.createAbilityItem(Material.CHAIN,
                "§c[A3] Hell's Pull", "Fire chain pull 3 dmg | 20s CD");
    ...
    public void activateAbility1(Player player) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            runLater(() -> {
                ...
                // Damage nearby
                List<LivingEntity> hit = AbilityUtils.getNearby(player.getLocation(), 2, player);
    ...
    private void executeWhipHit(Player player, int stage) {
        ...
        List<LivingEntity> hit = AbilityUtils.getNearby(player.getLocation().add(dir.clone().multiply(3)), 3, player);
    ...
    public void activateAbility3(Player player) {
        ...
        for (LivingEntity e : nearby) {
            ...
            for (int i = 0; i < (int) dist; i++) {
                ...
                world.spawnParticle(Particle.FLAME, chainLoc, 2, 0.1, 0.1, 0.1, 0.02);
            }
