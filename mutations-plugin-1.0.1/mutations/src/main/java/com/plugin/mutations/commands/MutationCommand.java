...
public class MutationCommand implements CommandExecutor, TabCompleter {
    ...
    /** Resolve a player name or @a selector into a list of online players. */
    private List<Player> resolveTargets(String arg) {
    ...
    private void handleSet(CommandSender sender, String[] args) {
        ...
        if (args.length < 3) { sender.sendMessage("§cUsage: /mutation set <player|@a> <mutation>"); return; }

        List<Player> targets = resolveTargets(args[1]);
        if (targets.isEmpty()) { sender.sendMessage("§cPlayer not found."); return; }
        ...
        String who = targets.size() == 1 ? "§f" + targets.get(0).getName() : "§f" + targets.size() + " players";
        sender.sendMessage("§aSet " + who + "§a's mutation to §f" + type.getDisplayName());
    }

    /** /mutation remove <player|@a> — remove mutation (no item returned) */
    private void handleRemove(CommandSender sender, String[] args) {
        if (!sender.hasPermission("mutations.admin")) { sender.sendMessage("§cNo permission."); return; }
        if (args.length < 2) { sender.sendMessage("§cUsage: /mutation remove <player|@a>"); return; }
        ...
        int removed = 0;
        for (Player target : targets) {
            if (!plugin.getMutationManager().hasMutation(target)) continue;
            plugin.getMutationManager().removeMutation(target);
            target.sendMessage("§cYour mutation has been removed.");
            removed++;
        }
        String who = targets.size() == 1 ? "§f" + targets.get(0).getName() : "§f" + removed + "/" + targets.size() + " players";
        sender.sendMessage("§aRemoved " + who + "§a's mutation.");
    }
    ...
    private void handleGive(CommandSender sender, String[] args) {
        ...
        plugin.getAdvancementManager().trackMutationSeen(target, type);
    ...
    private void handleWithdraw(CommandSender sender, String[] args) {
        ...
        // Return the orb — drop at feet if inventory full
        ItemStack orb = plugin.getMutationManager().getItemManager().createMutationItem(type);
        var leftover = target.getInventory().addItem(orb);
        plugin.getAdvancementManager().trackMutationSeen(target, type);
        if (!leftover.isEmpty()) {
            target.getWorld().dropItemNaturally(target.getLocation(), leftover.get(0));
            target.sendMessage("§eYour " + type.getDisplayName() + " §emutation has been withdrawn. §cInventory full — orb dropped at your feet.");
        } else {
    ...
    private String[] getCooldownKeys(MutationType type) {
        return switch (type) {
            ...
            case LIGHTNING -> new String[]{"A1_LightningRide", "A2_Siphon", "A3_Arena"};
            case NATURE    -> new String[]{"A1_NaturesGrasp", "A2_TrunkSlam", "A3_ViolentNature"};
            case INVENTORY -> new String[]{"A1_Scramble"};
        };
    }
    ...
    /** /mutation random <player|@a> — give a random pool mutation */
    private void handleRandom(CommandSender sender, String[] args) {
        if (!sender.hasPermission("mutations.admin")) { sender.sendMessage("§cNo permission."); return; }
        if (args.length < 2) { sender.sendMessage("§cUsage: /mutation random <player|@a>"); return; }

        List<Player> targets = resolveTargets(args[1]);
        if (targets.isEmpty()) { sender.sendMessage("§cPlayer not found."); return; }

        List<MutationType> pool = List.of(
                MutationType.WIND,
                MutationType.FROZEN,
                MutationType.ROCK,
        ...
        for (Player target : targets) {
            MutationType chosen = pool.get(rng.nextInt(pool.size()));
        ...
        String who = targets.size() == 1
                ? "§f" + targets.get(0).getName()
                : "§f" + targets.size() + " players";
        sender.sendMessage("§aGave " + who + " §aa random mutation.");
