...
public class FrozenMutation extends Mutation {
    ...
    public void activateAbility1(Player player) {
        ...
        // Visual ice effect using particles only (no block placement = no suffocation)
        world.spawnParticle(Particle.SNOWFLAKE, player.getLocation().add(0,1,0), 80, 0.6, 1, 0.6, 0.1);
        player.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 80, 10, false, false)); // frozen in place
        player.addPotionEffect(new PotionEffect(PotionEffectType.RESISTANCE, 80, 1, false, false)); // tanky while frozen

        // Heal 3 hearts over 3s (1 heart per second) using real hearts
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            player.setHealth(Math.min(player.getAttribute(org.bukkit.attribute.Attribute.MAX_HEALTH).getValue(), player.getHealth() + 2.0)); // heal 1 heart
