...
public class TrueShotMutation extends Mutation {
    ...
    public void activateAbility1(Player player) {
        if (!checkCooldown(player, "A1_PenVolley", 32)) return;
    ...
    public void activateAbility2(Player player) {
        if (!checkCooldown(player, "A2_GuidedLight", 36)) return;
    ...
    public void activateAbility3(Player player) {
        if (!checkCooldown(player, "A3_TrueShotApex", 50)) return;
        ...
        arrow.setDamage(6); // 3 hearts base (trueDamage in PassiveListener overrides to 3 hearts)
        arrow.setPickupStatus(AbstractArrow.PickupStatus.DISALLOWED);
        arrow.setGlowing(true);
