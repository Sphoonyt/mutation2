...
public class TrueShotMutation extends Mutation {
    ...
    public void activateAbility2(Player player) {
        ...
        // Homing behavior: runs every tick for strong predictive tracking
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            if (lockedTarget[0] != null && lockedTarget[0].isValid()) {
                ...
                // Aim at chest height with velocity-lead prediction
                Vector targetVel = t.getVelocity();
                Location aimSpot = t.getLocation().add(0, t.getHeight() * 0.6, 0);
                // Lead the target: predict where it'll be in ~3 ticks
                aimSpot.add(targetVel.clone().multiply(3));

                Vector toTarget = aimSpot.toVector().subtract(arrow.getLocation().toVector());
                Vector dir = toTarget.normalize();

                // Hard steer: just set velocity directly toward target every tick
                double speed = Math.max(2.8, arrow.getVelocity().length()); // never slow down
                arrow.setVelocity(dir.multiply(speed));
                arrow.setGravity(false); // disable gravity so it doesn't arc away
            }
        }, 0, 1);
    }
    ...
    public String[] getCooldownKeys() {
        return {"A1_PenVolley", "A2_GuidedLight", "A3_TrueShotApex"};
    }
