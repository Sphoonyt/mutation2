...
public class RockMutation extends Mutation {
    ...
    public void activateAbility1(Player player) {
        ...
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "rock_a1");
        ...
        for (LivingEntity e : nearby) {
            AbilityUtils.trueDamage(e, 4, player, "§8" + e.getName() + " §7was crushed by §8" + player.getName() + "§7's Stone Slam");
    ...
    public void activateAbility3(Player player) {
        ...
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "rock_a3");
    ...
    public String[] getCooldownKeys() {
        return new String[]{"A1_StoneSlamm", "A2_SkinHarden", "A3_BoulderBarrage"};
    }

    @Override
    public boolean isAbilityActive(int slot, java.util.UUID uuid) {
        // Harden active = player has Resistance II
        if (slot == 2) {
            org.bukkit.entity.Player p = plugin.getServer().getPlayer(uuid);
            if (p != null) {
                org.bukkit.potion.PotionEffect eff = p.getPotionEffect(org.bukkit.potion.PotionEffectType.RESISTANCE);
                return eff != null && eff.getAmplifier() >= 1;
            }
        }
        return false;
    }
