...
public class NatureMutation extends Mutation {
    ...
    public void activateAbility1(Player player) {
        ...
        for (Player target : targets) {
            ...
            // Stop healing: Mining Fatigue + Weakness suppress regen
            target.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS, 100, 0, false, false));
            // Cancel any active regen effect
            target.removePotionEffect(PotionEffectType.REGENERATION);

            // Keep cancelling regen for 5s via ticker
            plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
                if (!target.isOnline()) { task.cancel(); return; }
                target.removePotionEffect(PotionEffectType.REGENERATION);
            }, 0, 5L);
            // Stop after 5s (100 ticks)
            runLater(() -> {
                // task will naturally expire after 5s — no explicit cancel needed as regen returns
            }, 100L);

            target.sendMessage("§2🌿 You've been grasped by " + player.getName() + "! Healing suppressed for 5s!");
    ...
    public void activateAbility2(Player player) {
        ...
        AbilityUtils.trueDamage(target, 4, player);
    ...
    public void activateAbility3(Player player) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            AbilityUtils.trueDamage(t, 0.5, player);

            // Leaf particles on target
            world.spawnParticle(Particle.FALLING_LEAVES, t.getLocation().add(0,1,0),
                    6, 0.4, 0.6, 0.4, 0.05);
