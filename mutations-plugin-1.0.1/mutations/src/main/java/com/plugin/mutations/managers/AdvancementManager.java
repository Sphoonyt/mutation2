...
public class AdvancementManager {
    ...
    private static final Set<MutationType> MASTER_POOL = Set.of(
        ...
        MutationType.TRUE_SHOT, MutationType.INVENTORY
    );
    ...
    private void installDatapack() {
        ...
        // 1.21+ uses singular "advancement" folder name
        File advDir = new File(worldFolder, "datapacks/mutations/data/mutations/advancement/mutations");
        advDir.mkdirs();
        ...
        // Enable and reload the datapack on first tick after startup
    ...
    public void grant(Player player, String advancementId) {
        Bukkit.getScheduler().runTask(plugin, () -> {
            try {
                NamespacedKey key = NamespacedKey.fromString("mutations:mutations/" + advancementId);
                if (key == null) return;
                Advancement adv = Bukkit.getAdvancement(key);
                if (adv == null) {
                    plugin.getLogger().warning("[Mutations] Advancement not found: mutations:mutations/" + advancementId + " — is the datapack loaded?");
                    return;
                }
    ...
    public void grantCriterion(Player player, String advancementId, String criterionId) {
        Bukkit.getScheduler().runTask(plugin, () -> {
            try {
                NamespacedKey key = NamespacedKey.fromString("mutations:mutations/" + advancementId);
                if (key == null) return;
                ...
                player.getAdvancementProgress(adv).awardCriteria(criterionId);
            } catch (Exception e) {
                plugin.getLogger().log(Level.WARNING, "Failed to grant criterion " + criterionId + " on " + advancementId, e);
            }
        });
    }
    ...
    /** Call this whenever a player equips, receives, or picks up any mutation (orb or active). */
    public void trackMutationSeen(Player player, MutationType type) {
        if (!MASTER_POOL.contains(type)) return;
        UUID uuid = player.getUniqueId();
        Set<MutationType> set = obtainedMutations.computeIfAbsent(uuid, k -> new HashSet<>());
        if (set.contains(type)) return; // already tracked, skip criterion grant
        set.add(type);
        saveObtained();

        // Grant the individual criterion (shows X/13 fraction)
        String criterionId = switch (type) {
            ...
            case INVENTORY     -> "obtain_inventory";
            default            -> null;
        };
        if (criterionId != null) grantCriterion(player, "mutation_master", criterionId);
    }

    // Keep old name as alias so existing calls in MutationManager still compile
    public void onMutationObtained(Player player, MutationType type) {
        grant(player, "root");
        trackMutationSeen(player, type);
    }
    ...
    public void onDamagingAbilityUsed(Player player, String abilityKey) {
        ...
        set.add(abilityKey);
        // Grant the individual criterion so the fraction updates
        grantCriterion(player, "master_of_damage", abilityKey);
    }
