...
public class ShadowMutation extends Mutation {
    ...
    public void onRemove(Player player) {
        ...
        if (dissipateActive.remove(uuid) || hideSeekActive.contains(uuid)) {
            showToAll(player);
        }
    ...
    // ── Passive: Shadow Lurker — regular potion invis (hides body, not armor) ─
    @Override
    public void onTick(Player player) {
        player.addPotionEffect(new PotionEffect(
                PotionEffectType.INVISIBILITY, 40, 0, false, false), true);

        // Dark dust swirl — visible to others so they know your general location
        Location loc = player.getLocation().add(0, 1, 0);
        player.getWorld().spawnParticle(Particle.DUST, loc, 6, 0.5, 0.8, 0.5, 0,
                new Particle.DustOptions(Color.fromRGB(50, 0, 80), 2.5f));
        player.getWorld().spawnParticle(Particle.DUST, loc, 4, 0.3, 0.3, 0.3, 0,
                new Particle.DustOptions(Color.fromRGB(15, 0, 30), 3.0f));
    }

    // ── True invis helpers (hidePlayer — hides body AND armor/items) ─────────
    ...
    public void activateAbility1(Player player) {
        ...
        // Mask AoE with squid ink particles
        for (int r = 0; r < 80; r++) {
        ...
        // True invis: hide body + armor from all clients
        hideFromAll(player);
    ...
    public void onDamageReceived(Player player, EntityDamageByEntityEvent event) {
        UUID uuid = player.getUniqueId();
        // Getting hit breaks Hide and Seek (but NOT Dissipate)
        ...
        // Dissipate true invis does NOT break on hit — it just expires after 10s
    }
