...
public class ShadowMutation extends Mutation {
    ...
    public void onRemove(Player player) {
        dissipateActive.remove(player.getUniqueId());
        hideSeekActive.remove(player.getUniqueId());
        hideSeekUsedBackstab.remove(player.getUniqueId());
    ...
    /** Hides this player from every other online player completely (no model, no armor, no nametag) */
    ...
    /** Restores visibility to every other online player */
    ...
    // ── Passive: Shadow Lurker ────────────────────────────────────────────────
    @Override
    public void onTick(Player player) {
        // Re-hide from anyone who might have had visibility restored (e.g. relog)
        for (Player other : Bukkit.getOnlinePlayers()) {
            if (other == player) continue;
    ...
    public void activateAbility3(Player player) {
        ...
        // Regen 0.5 hearts (1 HP) every 10s
