...
public class ShadowMutation extends Mutation {
    ...
    public void onRemove(Player player) {
        ...
        trueInvis(player, false);
    }

    // ── Passive: Shadow Lurker — perm true invis ─────────────────────────────
    @Override
    public void onTick(Player player) {
        // setInvisible handles visibility for all clients — just ensure it stays on
    ...
    // ── Helpers ───────────────────────────────────────────────────────────────
    private void trueInvis(Player player, boolean hide) {
        // setInvisible hides body AND all equipped armor/items (unlike potion alone)
    ...
    public void activateAbility2(Player player) {
        ...
        runLater(() -> {
            ...
            if (target.isOnline()) {
                ...
                // Weakness for remainder of 4s (4s - 1.5s freeze = 2.5s left)
            }
        }, 30L);

        // Weakness for full 4s
    ...
    public void activateAbility3(Player player) {
        ...
        // Already hidden from passive — just ensure it's active
        trueInvis(player, true);

        // Regen 0.5 hearts per 10s = 1 HP per 200 ticks
        plugin.getServer().getScheduler().runTaskTimer(plugin, new Runnable() {
            @Override public void run() {
                if (!player.isOnline() || !hideSeekActive.contains(uuid)) {
                    this.hashCode(); // trigger cancel via outer task ref — handled below
                    return;
                }
        ...
        runLater(() -> {
            ...
            if (player.isOnline()) {
                ...
                // Re-apply passive invis (still have shadow mutation)
                trueInvis(player, true);
            }
        }, 200L);
    }
    ...
    public void onDamageDealt(Player player, LivingEntity target, EntityDamageByEntityEvent event) {
        ...
        if (hideSeekActive.contains(uuid) && !hideSeekUsedBackstab.contains(uuid)) {
            ...
            AbilityUtils.trueDamage(target, baseDmg / 2.0, player);
