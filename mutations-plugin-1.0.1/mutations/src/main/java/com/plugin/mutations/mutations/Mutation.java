...
public abstract class Mutation {
    ...
    /**
     * Returns the cooldown key for the given ability slot (1-indexed).
     * Used by the action bar manager to look up cooldown status.
     */
    public String[] getCooldownKeys() { return new String[]{}; }

    /**
     * Returns true if the given ability slot (1-indexed) is currently active (mid-duration).
     * Override in subclasses that track active state.
     */
    public boolean isAbilityActive(int slot, java.util.UUID uuid) { return false; }
    ...
    protected org.bukkit.scheduler.BukkitTask runLaterTask(Runnable task, long ticks) {
        return plugin.getServer().getScheduler().runTaskLater(plugin, task, ticks);
    }
