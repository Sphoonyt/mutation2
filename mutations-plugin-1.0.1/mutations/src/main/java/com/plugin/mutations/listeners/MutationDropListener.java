...
public class MutationDropListener implements Listener {
    ...
    // ─── WIND — OMINOUS VAULT (VaultChangeStateEvent, LootGenerateEvent
    //     doesn't fire for vaults in Paper 1.21.x) ─────────────────────────

    @EventHandler
    public void onVaultEject(VaultChangeStateEvent event) {
        // Only fire when vault transitions to EJECTING (loot pops out)
        if (event.getNewState() != Vault.State.EJECTING) return;

        // Check it's ominous
        if (!(event.getBlock().getBlockData() instanceof Vault vaultData)) return;
        if (!vaultData.isOminous()) return;

        Player player = event.getPlayer();
        if (player == null) return;

        if (random.nextDouble() < 0.10) {
            giveMutation(player, MutationType.WIND);
        }
    }
