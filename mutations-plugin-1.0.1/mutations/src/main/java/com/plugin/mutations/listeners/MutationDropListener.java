...
public class MutationDropListener implements Listener {
    ...
    private boolean isOnAcquisitionCooldown(Player player, MutationType type) {
        ...
        boolean onCooldown = System.currentTimeMillis() - last < COOLDOWN_MS;
        if (onCooldown) plugin.getAdvancementManager().onCooldownHit(player);
        return onCooldown;
    }
    ...
    private void giveMutation(Player player, MutationType type) {
        if (isOnAcquisitionCooldown(player, type)) {
            ...
            plugin.getAdvancementManager().grant(player, "i_dont_think_so");
            return;
        }
    ...
    public void onWitherDeath(EntityDeathEvent event) {
        ...
        if (killer != null) {
            ...
            plugin.getAdvancementManager().grant(killer, "wither_slayer");
        }
    ...
    public void onVoidDeath(PlayerDeathEvent event) {
        ...
        // Always grant deeper_than_dark for dying to void
        plugin.getAdvancementManager().grant(player, "deeper_than_dark");
        // Shadow orb only if they have no mutation
    ...
    public void onRespawnAfterVoid(PlayerRespawnEvent event) {
        ...
        plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
            ...
            plugin.getAdvancementManager().onVoidDeath(player);
        }, 5L);
    }
    ...
    public void onSkeletonKilledByArrow(EntityDeathEvent event) {
        ...
        plugin.getAdvancementManager().grant(shooter, "long_shot");
    }
    ...
    public void onWorldBorderDamage(EntityDamageEvent event) {
        ...
        if (!borderDamageStart.containsKey(uuid)) {
            ...
            if (elapsed >= 60_000L) {
                ...
                plugin.getAdvancementManager().onEdgeWalker(player);
            }
        }
    }
