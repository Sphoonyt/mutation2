...
public class HellfireMutation extends Mutation {
    ...
    private final Map<UUID, Long>    whipLastUse = new HashMap<>();
    ...
    public void onRemove(Player player) {
        ...
        whipLastUse.remove(player.getUniqueId());
    }
    ...
    public void activateAbility1(Player player) {
        ...
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "hellfire_a1");
        ...
        for (int i = 0; i < 3; i++) {
            ...
            runLater(() -> {
                ...
                // Break cobwebs in 5x5x5 area around player
                Location center = player.getLocation();
                for (int bx = -2; bx <= 2; bx++) {
                    for (int by = -2; by <= 2; by++) {
                        for (int bz = -2; bz <= 2; bz++) {
                            Block b = center.getBlock().getRelative(bx, by, bz);
                ...
                for (LivingEntity e : hit) {
                    AbilityUtils.trueDamage(e, 2.0, player, "§c" + e.getName() + " §7was incinerated by §c" + player.getName() + "§7's Hellfire Rush");
    ...
    public void activateAbility2(Player player) {
        ...
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "hellfire_a2");
            whipStage.put(uuid, 1);
            whipLastUse.put(uuid, System.currentTimeMillis());
            player.sendMessage("§c🔥 Flame Whip! [1/3] - Chain within 3s, wait 1s for triple!");
            executeWhipHit(player, 0);
            whipStage.put(uuid, 1);

            runLater(() -> {
                if (whipStage.getOrDefault(uuid, 0) == 1) whipStage.put(uuid, 0);
            }, 60L);
            return;
        }
        ...
        if (stage < 3) {
            ...
            long lastUse = whipLastUse.getOrDefault(uuid, 0L);
            boolean tripleHit = (now - lastUse) >= 1000L; // waited at least 1s
            whipLastUse.put(uuid, now);

            executeWhipHit(player, stage, tripleHit);
            whipStage.put(uuid, stage + 1);

            if (stage + 1 >= 3) {
                whipStage.put(uuid, 0);
                whipLastUse.remove(uuid);
            } else {
                runLater(() -> {
    ...
    private void executeWhipHit(Player player, int stage) {
        executeWhipHit(player, stage, false);
    }

    private void executeWhipHit(Player player, int stage, boolean triple) {
        World world = player.getWorld();
        ...
        if (stage < 2) {
            // First 2 hits: 0.5 hearts (1.5 if triple)
            double dmg = triple ? 1.5 : 0.5;
            for (LivingEntity e : hit) {
                AbilityUtils.trueDamage(e, dmg, player, "§c" + e.getName() + " §7was lashed by §c" + player.getName() + "§7's Flame Whip");
            }
            world.playSound(player.getLocation(), Sound.ENTITY_BLAZE_SHOOT, 0.6f, 1.2f);
            String tripleMsg = triple ? " §6[TRIPLE!]" : "";
            player.sendMessage("§c🔥 Whip hit " + (stage + 1) + "!" + tripleMsg + " §7(" + (2 - stage) + " more)");
        } else {
            // Final hit: 1 heart (3 if triple)
            double dmg = triple ? 3.0 : 1.0;
            for (LivingEntity e : hit) {
                AbilityUtils.trueDamage(e, dmg, player, "§c" + e.getName() + " §7was obliterated by §c" + player.getName() + "§7's Flame Whip Slam");
            }
            String tripleMsg = triple ? " §6[TRIPLE SLAM!]" : "";
            player.sendMessage("§c🔥 Final Flame Whip Slam!" + tripleMsg);
            for (int i = 1; i <= 8; i++) {
    ...
    public void activateAbility3(Player player) {
        ...
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "hellfire_a3");
        ...
        for (LivingEntity target : nearby) {
            ...
            plugin.getServer().getScheduler().runTaskTimer(plugin, hookTask -> {
                ...
                if (tick[0] == totalTicks - 1) {
                    ...
                    AbilityUtils.trueDamage(finalTarget, 1.5, player, "§c" + finalTarget.getName() + " §7was dragged into the inferno by §c" + player.getName() + "§7's Hell's Pull");
