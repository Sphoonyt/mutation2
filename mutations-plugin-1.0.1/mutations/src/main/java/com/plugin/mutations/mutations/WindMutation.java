...
public class WindMutation extends Mutation {
    ...
    private final Set<UUID>          rechargingPlayers = new HashSet<>();
    ...
    public void activateAbility1(Player player) {
        ...
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "wind_a1");
        player.sendMessage("§b🌪️ Tornado Eruption!");

        Location loc = player.getLocation();
        World world = player.getWorld();
        world.playSound(loc, Sound.ENTITY_WITHER_SHOOT, 1f, 0.5f);
        world.spawnParticle(Particle.CLOUD, loc.clone().add(0, 1, 0), 100, 1.8, 1.8, 1.8, 0.12);
        world.spawnParticle(Particle.END_ROD, loc.clone().add(0,1,0), 30, 1.2, 1.2, 1.2, 0.05);
        world.spawnParticle(Particle.SWEEP_ATTACK, loc, 20, 1.2, 0.5, 1.2, 0.1);
        ...
        runLater(() -> {
            ...
            world.spawnParticle(Particle.EXPLOSION_EMITTER, player.getLocation(), 8, 1.2, 1.2, 1.2, 0.1);
            world.spawnParticle(Particle.CLOUD, player.getLocation().add(0,0.5,0), 50, 1.5, 0.5, 1.5, 0.1);
            ...
            for (LivingEntity target : targets) {
                ...
                AbilityUtils.trueDamage(target, 1.5, player, "§b" + target.getName() + " §7was swept away by §b" + player.getName() + "§7's Tornado");
            }
        }, 20L);
    }
    ...
    public void activateAbility2(Player player) {
        ...
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "wind_a2");
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            world.spawnParticle(Particle.CLOUD,   sb.getLocation(), 5,  0.1, 0.1, 0.1, 0.02);
            world.spawnParticle(Particle.END_ROD, sb.getLocation(), 3,  0.05, 0.05, 0.05, 0.01);
        }, 0, 1);
    }
    ...
    public void activateAbility3(Player player) {
        ...
        world.spawnParticle(Particle.CLOUD,   player.getLocation(), 40, 0.6, 0.2, 0.6, 0.12);
        world.spawnParticle(Particle.END_ROD, player.getLocation(), 15, 0.4, 0.1, 0.4, 0.05);
    ...
    public void onTick(Player player) {
        ...
        if (player.isOnGround()) {
            boolean usedJumps = airJumpsLeft.getOrDefault(uuid, MAX_AIR_JUMPS) < MAX_AIR_JUMPS;
            boolean onCooldown = plugin.getCooldownManager().isOnCooldown(uuid, "WindJumpReset");

            if (usedJumps && !onCooldown && !rechargingPlayers.contains(uuid)) {
                // Just landed after using jumps — start 5s cooldown
                plugin.getCooldownManager().setCooldown(uuid, "WindJumpReset", 5);
                rechargingPlayers.add(uuid);
                player.sendMessage("§b🌪️ Air Step recharging... (5s)");
            } else if (rechargingPlayers.contains(uuid) && !onCooldown) {
                // Cooldown just expired — restore jumps
                rechargingPlayers.remove(uuid);
                airJumpsLeft.put(uuid, MAX_AIR_JUMPS);
                player.sendMessage("§b🌪️ Air Step ready!");
            } else if (!usedJumps) {
                // No jumps used, ensure clean state
                rechargingPlayers.remove(uuid);
            }
    ...
    public boolean tryAirJump(Player player) {
        ...
        player.getWorld().spawnParticle(Particle.CLOUD,   player.getLocation(), 20, 0.4, 0.1, 0.4, 0.06);
        player.getWorld().spawnParticle(Particle.END_ROD, player.getLocation(), 8,  0.3, 0.1, 0.3, 0.02);
