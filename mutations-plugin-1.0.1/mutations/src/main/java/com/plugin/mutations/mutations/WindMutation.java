...
public class WindMutation extends Mutation {
    ...
    public void onTick(Player player) {
        ...
        if (player.isOnGround()) {
            // Landed — start 5s cooldown before jumps restore
            if (airJumpsLeft.getOrDefault(uuid, MAX_AIR_JUMPS) < MAX_AIR_JUMPS) {
                // Only set cooldown if we actually used jumps
                if (!plugin.getCooldownManager().isOnCooldown(uuid, "WindJumpReset")) {
