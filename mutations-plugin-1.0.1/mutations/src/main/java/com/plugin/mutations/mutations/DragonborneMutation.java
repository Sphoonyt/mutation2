...
public class DragonborneMutation extends Mutation {
    ...
    private void venomBurst(Player player) {
        ...
        for (LivingEntity e : nearby) {
            AbilityUtils.trueDamage(e, 2, player, "§2" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was poisoned by §2" + player.getName() + "§7's Venom Burst");
        ...
        runLater(() -> {
            ...
            for (LivingEntity e : finalHit) {
                AbilityUtils.trueDamage(e, 5, player, "§2" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was dissolved by §2" + player.getName() + "§7's Final Venom Burst");
    ...
    private void serpentGlide(Player player) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            for (LivingEntity e : hit) {
                AbilityUtils.trueDamage(e, 0.5, player, "§2" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was consumed by §2" + player.getName() + "§7's Serpent Glide");
    ...
    private void blazingEruption(Player player) {
        ...
        for (LivingEntity e : nearby) {
            AbilityUtils.trueDamage(e, 4, player, "§c" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was incinerated by §c" + player.getName() + "§7's Blazing Eruption");
    ...
    private void spawnFireGround(Player player, Location center, int radius, long durationTicks) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            for (LivingEntity e : hit) AbilityUtils.trueDamage(e, 0.5, player, "§c" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was burned by §c" + player.getName() + "§7's Inferno Leap");
        }, 0, 20L);
    ...
    private void ironcladSurge(Player player) {
        ...
        runLater(() -> {
            ...
            for (LivingEntity e : nearby) {
                AbilityUtils.trueDamage(e, 5, player, "§8" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was shattered by §8" + player.getName() + "§7's Fragment Burst");
    ...
    private void steelFistBarrage(Player player) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            runLater(() -> {
                ...
                for (LivingEntity e : hit) {
                    if (punch < 2) {
                        AbilityUtils.trueDamage(e, 2.5, player, "§8" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was pummelled by §8" + player.getName() + "§7's Steel Fist Barrage");
                        ...
                        AbilityUtils.trueDamage(e, 2, player, "§8" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was launched by §8" + player.getName() + "§7's Steel Fist Barrage");
    ...
    private void armorConstruct(Player player) {
        ...
        runLater(() -> {
            ...
            for (LivingEntity e : hit) AbilityUtils.trueDamage(e, 2, player, "§8" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was crushed by §8" + player.getName() + "§7's Armor Construct");
    ...
    public boolean tryAirJump(Player player) {
        ...
        if (style == Style.POISON) {
            ...
            List<LivingEntity> hit = AbilityUtils.getNearbyTrusted(player.getLocation(), 3, player, plugin);
            for (LivingEntity e : hit) {
                AbilityUtils.trueDamage(e, 1, player, "§2" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was envenomed by §2" + player.getName() + "§7's Dragon Venom");
            ...
            List<LivingEntity> hit = AbilityUtils.getNearbyTrusted(player.getLocation(), 3, player, plugin);
            for (LivingEntity e : hit) {
                AbilityUtils.trueDamage(e, 1, player, "§c" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was scorched by §c" + player.getName() + "§7's Dragon Flame");
                e.setFireTicks(20);
            }
        }
    ...
    public String[] getCooldownKeys() {
        return style == Style.POISON ? new String[]{"A1_VenomBurst","A2_ToxicFang","A3_SerpentGlide"} : style == Style.FIRE ? new String[]{"A1_BlazingErupt","A2_FlameTalon","A3_InfernoLeap"} : new String[]{"A1_Ironclad","A2_SteelFist","A3_ArmorConstruct"};
    }

    @Override
    public boolean isAbilityActive(int slot, java.util.UUID uuid) {
        // Construct (Armor A3) active tracking
        if (style == Style.ARMOR && slot == 3) return constructStands.containsKey(uuid);
        return false;
    }
