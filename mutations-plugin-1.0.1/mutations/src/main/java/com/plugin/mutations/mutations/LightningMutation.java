...
public class LightningMutation extends Mutation {
    ...
    public void activateAbility1(Player player) {
        ...
        taskHolder[0] = plugin.getServer().getScheduler().runTaskTimer(plugin, new Runnable() {
            @Override public void run() {
                ...
                // Drive player at 15 blocks/second (0.75/tick)
                Vector dir = player.getEyeLocation().getDirection().normalize().multiply(0.75);
                ...
                // Lightning effect only (no damage to rider) every 10 ticks
                if (tickCount[0] % 10 == 0) {
                    world.strikeLightningEffect(player.getLocation());
    ...
    public void activateAbility3(Player player) {
        ...
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "lightning_a3");
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                for (LivingEntity e : AbilityUtils.getNearbyTrusted(strike, 1.5, player, plugin)) {
                    AbilityUtils.trueDamage(e, 0.5, player, "§e" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was struck by §e" + player.getName() + "§7's Lightning Arena");
                }
            }
