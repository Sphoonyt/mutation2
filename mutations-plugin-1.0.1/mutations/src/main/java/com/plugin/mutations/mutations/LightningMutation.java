...
public class LightningMutation extends Mutation {
    ...
    public void activateAbility1(Player player) {
        ...
        // Spawn an invisible Phantom as the rideable vehicle
        // Phantom can fly and be controlled by passenger direction
        Phantom vehicle = (Phantom) world.spawnEntity(player.getLocation(), EntityType.PHANTOM);
        ...
        boltVehicle.put(uuid, vehicle.getUniqueId());

        // Tick: move vehicle in player look direction at high speed + lightning FX
        ...
        taskHolder[0] = plugin.getServer().getScheduler().runTaskTimer(plugin, new Runnable() {
            @Override public void run() {
                ...
                Entity v = getVehicle(uuid, world);
                if (v == null) { stopBolt(player); taskHolder[0].cancel(); return; }

                // Move vehicle in player look direction — 4x happy ghast speed (~4.0 blocks/tick)
                Vector dir = player.getEyeLocation().getDirection().normalize().multiply(4.0);
                v.setVelocity(dir);

                // Lightning + spark particles around player
                ...
                // Strike lightning effect at vehicle location every 10 ticks
                if (tickCount[0] % 10 == 0) {
                    world.strikeLightningEffect(v.getLocation());
                    world.playSound(v.getLocation(), Sound.ENTITY_LIGHTNING_BOLT_THUNDER, 0.6f, 1.5f);
                }
            }
        }, 0L, 1L);
    }
    ...
    public void onDamageReceived(Player player, EntityDamageByEntityEvent event) {
        if (boltActive.contains(player.getUniqueId())) {
            stopBolt(player);
    ...
    private void stopBolt(Player player) {
        ...
        UUID vehicleUUID = boltVehicle.remove(uuid);
        if (vehicleUUID != null) {
            for (World w : Bukkit.getWorlds()) {
                Entity e = w.getEntities().stream()
                        .filter(en -> en.getUniqueId().equals(vehicleUUID))
        ...
        // Teleport player to ground safely
        if (player.isOnline()) {
            Location safe = player.getLocation().clone();
            safe.setY(player.getWorld().getHighestBlockYAt(safe) + 1);
