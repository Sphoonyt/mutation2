...
public class AbilityCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        // Exception: skip if player is mid-whip combo (Hellfire A2 chaining)
        UUID uuid = player.getUniqueId();
        long now = System.currentTimeMillis();
        long lastUsed = globalCooldown.getOrDefault(uuid, 0L);
        long remaining = GLOBAL_CD_MS - (now - lastUsed);
        boolean midWhipCombo = "2".equals(args[0])
                && mutation instanceof com.plugin.mutations.mutations.HellfireMutation hm
                && hm.isWhipActive(uuid);
        if (remaining > 0 && !midWhipCombo) {
            player.sendMessage("§7⏳ Ability on cooldown — wait §c" + (remaining / 1000 + 1) + "s§7.");
            return true;
        }
        if (!midWhipCombo) globalCooldown.put(uuid, now);
