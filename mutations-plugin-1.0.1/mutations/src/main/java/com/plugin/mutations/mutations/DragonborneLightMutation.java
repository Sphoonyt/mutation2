...
public class DragonborneLightMutation extends Mutation {

    private final Map<UUID, Long> lastBlindChance = new HashMap<>();
    private final Random random = new Random();

    public DragonborneLightMutation(MutationsPlugin plugin) {
        super(plugin);
    }

    @Override
    public MutationType getType() { return MutationType.DRAGONBORNE_LIGHT; }

    @Override
    public void onAssign(Player player) {
        player.sendMessage("§e☀️ You have been granted the §eDragonborne (Light)§e!");
    ...
    public void activateAbility1(Player player) {
        if (!checkCooldown(player, "A1_LuminousRoar", 27)) return;
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "dragonborne_light_a1");
        ...
        // 12-block beam
        Set<LivingEntity> hit = AbilityUtils.beamDamage(player, 12, 1.2, Particle.END_ROD, null, 2,
                "§e%victim% §7was vaporised by §e" + player.getName() + "§7's Luminous Roar");
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            for (LivingEntity e : trail) {
                AbilityUtils.trueDamage(e, 0.5, player, "§e" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was burned by §e" + player.getName() + "§7's Luminous Roar");
    ...
    public void activateAbility2(Player player) {
        if (!checkCooldown(player, "A2_SolarWing", 18)) return;
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "dragonborne_light_a2");
        ...
        runLater(() -> {
            ...
            for (LivingEntity e : nearby) {
                AbilityUtils.trueDamage(e, 2, player, "§e" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was blasted by §e" + player.getName() + "§7's Solar Wing");
    ...
    public void activateAbility3(Player player) {
        if (!checkCooldown(player, "A3_CelestialExp", 33)) return;
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            if (!player.isOnline() || tick[0] >= 80) {
                ...
                for (LivingEntity e : finalHit) {
                    AbilityUtils.trueDamage(e, 5, player, "§e" + (e instanceof Player ep ? ep.getName() : e.getName()) + " §7was annihilated by §e" + player.getName() + "§7's Solar Wing Strike");
