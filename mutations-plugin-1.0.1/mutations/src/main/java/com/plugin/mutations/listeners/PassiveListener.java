...
public class PassiveListener implements Listener {
    ...
    /** Apply double max stack when player picks up items */
    @EventHandler
    public void onInvPickup(org.bukkit.event.entity.EntityPickupItemEvent event) {
        if (!(event.getEntity() instanceof Player p)) return;
        if (!InventoryMutation.isActive(p.getUniqueId())) return;
        InventoryMutation.applyDoubleStack(p, event.getItem().getItemStack());
    }

    /** Apply double stack when player clicks items into inventory */
    @EventHandler
    public void onInvClick(org.bukkit.event.inventory.InventoryClickEvent event) {
        if (!(event.getWhoClicked() instanceof Player p)) return;
        if (!InventoryMutation.isActive(p.getUniqueId())) return;
        // When dropping into player inventory, apply double stack
        if (event.getClickedInventory() == p.getInventory() && event.getCurrentItem() != null) {
            InventoryMutation.applyDoubleStack(p, event.getCurrentItem());
        }
    }

    /** Revert stacks when items are dropped on the ground */
    @EventHandler
    public void onInvItemDrop(org.bukkit.event.player.PlayerDropItemEvent event) {
        Player p = event.getPlayer();
        if (!InventoryMutation.isActive(p.getUniqueId())) return;
        ItemStack dropped = event.getItemDrop().getItemStack();
        int vanillaMax = dropped.getType().getMaxStackSize();
        { org.bukkit.inventory.meta.ItemMeta _meta_ = dropped.getItemMeta(); if (_meta_ != null) { _meta_.setMaxStackSize(vanillaMax); dropped.setItemMeta(_meta_); } }
        // Revert rest of inventory too
        InventoryMutation.revertInventory(p);
        // Re-apply double stack to remaining inventory items
        for (ItemStack item : p.getInventory().getContents()) {
            InventoryMutation.applyDoubleStack(p, item);
        }
    }

    /** Revert stacks when player puts item into a container (chest, etc.) */
    @EventHandler
    public void onInvClickContainer(org.bukkit.event.inventory.InventoryClickEvent event2) {
        if (!(event2.getWhoClicked() instanceof Player p)) return;
        if (!InventoryMutation.isActive(p.getUniqueId())) return;
        if (event2.getClickedInventory() == null) return;
        // If clicking into a non-player inventory (chest/hopper/etc), revert the cursor item
        if (event2.getClickedInventory() != p.getInventory()) {
            ItemStack cursor = event2.getCursor();
            if (cursor != null && cursor.getType() != org.bukkit.Material.AIR) {
                org.bukkit.inventory.meta.ItemMeta cm = cursor.getItemMeta();
                if (cm != null) { cm.setMaxStackSize(cursor.getType().getMaxStackSize()); cursor.setItemMeta(cm); }
            }
        }
    }

    /** Revert inventory on death (mutation remove handles this, but just in case) */
    @EventHandler
    public void onInvDeath(org.bukkit.event.entity.PlayerDeathEvent event2) {
        Player p = event2.getEntity();
        if (!InventoryMutation.isActive(p.getUniqueId())) return;
        // Revert all drops to vanilla stack sizes
        for (ItemStack drop : event2.getDrops()) {
            if (drop == null) continue;
            org.bukkit.inventory.meta.ItemMeta dm = drop.getItemMeta();
            if (dm != null) { dm.setMaxStackSize(drop.getType().getMaxStackSize()); drop.setItemMeta(dm); }
        }
    }
}
