...
public class PassiveListener implements Listener {
    ...
    public void onOrbItemDamage(org.bukkit.event.entity.EntityDamageEvent event) {
        ...
        if (plugin.getMutationManager().getItemManager().isMutationItem(is)
                ...
                || plugin.getWandManager().isWand(is)) {
            event.setCancelled(true);
        }
    }
    ...
    public void onOrbSpawn(org.bukkit.event.entity.ItemSpawnEvent event) {
        ...
        if (!plugin.getMutationManager().getItemManager().isMutationItem(spawnedIs)
                && !plugin.getMaceManager().isMace(spawnedIs)
                && !plugin.getWandManager().isWand(spawnedIs)) return;
    ...
    // Revert fake display when player opens a container so server-client is in sync
    ...
    public void onInvOpen(org.bukkit.event.inventory.InventoryOpenEvent event) {
        ...
        // Revert to real values while container is open
        com.plugin.mutations.utils.OverstackUtil.revertDisplay(p);
    }
    ...
    public void onInvClose(org.bukkit.event.inventory.InventoryCloseEvent event) {
        if (!(event.getPlayer() instanceof Player p)) return;
        if (!InventoryMutation.isActive(p.getUniqueId())) return;
        // Re-apply fake display after container closes (1 tick delay for sync)
        plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
            if (p.isOnline() && InventoryMutation.isActive(p.getUniqueId())) {
                com.plugin.mutations.utils.OverstackUtil.applyDoubleStackDisplay(p);
            }
        }, 1L);
    }
}
