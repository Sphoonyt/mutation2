...
public class PassiveListener implements Listener {
    ...
    public void onRottenFleshRightClick(PlayerInteractEvent event) {
        ...
        // Prevent double-fire (HAND + OFF_HAND)
        UUID uuid = player.getUniqueId();
        ...
        // Cancel vanilla eating so we control the timing
        event.setCancelled(true);
        ...
        final int[] tick = {0};
        org.bukkit.scheduler.BukkitTask eatTask = plugin.getServer().getScheduler().runTaskTimer(plugin, t -> {
