...
public class PassiveListener implements Listener {
    ...
    public void onPlayerDeath(org.bukkit.event.entity.PlayerDeathEvent event) {
        ...
        // Only Dragonborne mutations are affected by death
        boolean isDroppable = type == MutationType.DRAGONBORNE_POISON
                || type == MutationType.DRAGONBORNE_FIRE
                || type == MutationType.DRAGONBORNE_ARMOR;

        if (!isDroppable) return; // all other mutations are completely unchanged on death

        // Remove the Dragonborne mutation from the victim
    ...
    // When a new player joins, ensure Shadow players appear invisible to them
    @EventHandler
    public void onPlayerJoin(PlayerJoinEvent event) {
        plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
            ...
            for (Player other : plugin.getServer().getOnlinePlayers()) {
                ...
                Mutation m = plugin.getMutationManager().getMutation(other);
                if (m instanceof ShadowMutation && !other.isInvisible()) {
