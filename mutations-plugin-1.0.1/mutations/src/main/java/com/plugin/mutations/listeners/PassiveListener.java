...
public class PassiveListener implements Listener {
    ...
    /** Ensure double-stack is applied whenever a player with this mutation picks up an item */
    @EventHandler
    public void onInvPickup(org.bukkit.event.entity.EntityPickupItemEvent event) {
        ...
        // Inventory max is already set to 99 — just double the picked-up amount if possible
        ItemStack item = event.getItem().getItemStack();
        int vanillaMax = item.getType().getMaxStackSize();
        if (vanillaMax > 1) {
            // Allow stacking up to double vanilla max by bumping inventory limit
            InventoryMutation.applyDoubleStack(p);
        }
    }

    /** When player drops an item, trim it to vanilla max and revert excess */
    @EventHandler
    public void onInvItemDrop(org.bukkit.event.player.PlayerDropItemEvent event) {
        ...
        if (dropped.getAmount() > vanillaMax) {
            // Drop excess as a second item, keep dropped at vanilla max
            int excess = dropped.getAmount() - vanillaMax;
            dropped.setAmount(vanillaMax);
            ItemStack extra = dropped.clone();
            extra.setAmount(excess);
            p.getWorld().dropItemNaturally(p.getLocation(), extra);
        }
    }

    /** When player puts items into a chest/container, trim oversized stacks */
    @EventHandler
    public void onInvClickContainer(org.bukkit.event.inventory.InventoryClickEvent event) {
        if (!(event.getWhoClicked() instanceof Player p)) return;
        if (!InventoryMutation.isActive(p.getUniqueId())) return;
        if (event.getClickedInventory() == null) return;
        if (event.getClickedInventory() == p.getInventory()) return;
        // Moving item INTO a non-player inventory — trim cursor item to vanilla max
        ItemStack cursor = event.getCursor();
        if (cursor != null && cursor.getType() != org.bukkit.Material.AIR) {
            int vanillaMax = cursor.getType().getMaxStackSize();
            if (cursor.getAmount() > vanillaMax) {
                int excess = cursor.getAmount() - vanillaMax;
                cursor.setAmount(vanillaMax);
                // Drop excess at player feet
                p.getWorld().dropItemNaturally(p.getLocation(), cursor.clone() /* already trimmed */ );
            }
        }
    }

    /** Revert drop amounts to vanilla max on death */
    @EventHandler
    public void onInvDeath(org.bukkit.event.entity.PlayerDeathEvent event) {
        Player p = event.getEntity();
        if (!InventoryMutation.isActive(p.getUniqueId())) return;
        for (ItemStack drop : event.getDrops()) {
            if (drop == null) continue;
            int vanillaMax = drop.getType().getMaxStackSize();
            if (drop.getAmount() > vanillaMax) drop.setAmount(vanillaMax);
        }
    }
}
