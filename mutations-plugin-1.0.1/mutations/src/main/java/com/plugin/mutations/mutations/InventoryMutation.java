...
public class InventoryMutation extends Mutation {
    ...
    public static void applyDoubleStack(Player player, ItemStack item) {
        ...
        int newMax = Math.min(vanillaMax * 2, 127);
        org.bukkit.inventory.meta.ItemMeta m = item.getItemMeta();
        if (m != null) { m.setMaxStackSize(newMax); item.setItemMeta(m); }
    }
    ...
    public static void revertInventory(Player player) {
        for (ItemStack item : player.getInventory().getContents()) {
            ...
            { org.bukkit.inventory.meta.ItemMeta _meta_ = item.getItemMeta(); if (_meta_ != null) { _meta_.setMaxStackSize(vanillaMax); item.setItemMeta(_meta_); } }
