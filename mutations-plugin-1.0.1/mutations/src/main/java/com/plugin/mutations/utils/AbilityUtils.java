...
public class AbilityUtils {
    ...
    public static void trueDamage(LivingEntity entity, double hearts, Player source) {
        ...
        // Drain absorption first — true damage should NOT bypass absorption hearts
        ...
        if (dmg <= 0) {
            // Fully absorbed — just play hit effects
        ...
        if (newHp <= 0.0) {
            // Check for totem: players only
            if (entity instanceof Player player && hasTotem(player)) {
                // Drop to 1 HP then fire a tiny real damage event so Bukkit triggers the totem
                entity.setHealth(1.0);
                entity.damage(1.0, source);
            } else {
                // No totem — kill directly so drops and death events fire
                entity.setHealth(0);
            }
        } else {
        ...
        for (int i = 0; i < length; i++) {
            ...
            for (Entity e : loc.getWorld().getNearbyEntities(loc, hitRadius, hitRadius, hitRadius)) {
                if (e instanceof LivingEntity le && e != source && !hit.contains(le)) {
                    trueDamage(le, heartsPerHit, source);
                    hit.add(le);
                }
            }
        }
