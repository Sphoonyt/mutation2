...
public class BloodSoldierMutation extends Mutation {
    ...
    private final Map<UUID, ItemStack> savedMainHand = new HashMap<>();
    private final Map<UUID, String> katanaIds = new HashMap<>(); // UUID → unique katana PDC id
    ...
    private final Map<UUID, org.bukkit.scheduler.BukkitTask> harvestResetTask = new HashMap<>();
    ...
    public void onRemove(Player player) {
        ...
        savedMainHand.remove(uuid);
        katanaIds.remove(uuid);
        nauseaReady.remove(uuid);
        org.bukkit.scheduler.BukkitTask ht = harvestResetTask.remove(uuid);
    ...
    public void activateAbility1(Player player) {
        ...
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "blood_a1");
        ...
        world.spawnParticle(Particle.FALLING_LAVA, player.getLocation().add(0,1,0), 60, 0.7, 0.7, 0.7, 0.07);
        world.spawnParticle(Particle.CRIT, player.getLocation().add(0,1,0), 25, 0.5, 0.8, 0.5, 0.1);
        world.spawnParticle(Particle.DUST, player.getLocation().add(0,1,0), 20, 0.5, 0.8, 0.5, 0, new Particle.DustOptions(Color.fromRGB(180,0,0), 1.5f));
    ...
    public void activateAbility3(Player player) {
        ...
        // Find an existing sword in inventory to replace; fall back to main hand
        int swordSlot = -1;
        ...
        if (swordSlot == -1) swordSlot = player.getInventory().getHeldItemSlot();

        ItemStack previous = player.getInventory().getItem(swordSlot);
        savedMainHand.put(uuid, previous != null ? previous.clone() : new ItemStack(Material.AIR));

        // Build the Blood Katana item — Sharpness V + netherite axe attack damage (10 dmg = 5 hearts)
        ...
        // Embed a unique ID so we can find this exact katana even if it moves slots
        String katanaId = java.util.UUID.randomUUID().toString();
        meta.getPersistentDataContainer().set(
            new org.bukkit.NamespacedKey(plugin, "blood_katana_id"),
            org.bukkit.persistence.PersistentDataType.STRING, katanaId);
        katanaIds.put(uuid, katanaId);
        ...
        meta.addAttributeModifier(
            ...
            new AttributeModifier(
                new org.bukkit.NamespacedKey(plugin, "blood_katana_dmg"),
                10.0,
        ...
        meta.addAttributeModifier(
            ...
            new AttributeModifier(
                new org.bukkit.NamespacedKey(plugin, "blood_katana_spd"),
                -2.4,
                AttributeModifier.Operation.ADD_NUMBER,
                org.bukkit.inventory.EquipmentSlotGroup.MAINHAND
            )
        ...
        player.getInventory().setItem(swordSlot, katana);
        player.getInventory().setHeldItemSlot(swordSlot);
        ...
        runLater(() -> {
            if (player.isOnline()) {
                katanaActive.remove(uuid);
                removeKatanaFromInventory(player, uuid);
                AbilityUtils.undoMaxHeartChange(player, -1.5);
    ...
    public void onDamageDealt(Player player, LivingEntity target, EntityDamageByEntityEvent event) {
        ...
        if (katanaActive.contains(uuid)) {
            ...
            // Drain 1 hunger bar from the attacker per hit
    ...
    private void triggerBloodHarvest(Player player, LivingEntity target) {
        ...
        AbilityUtils.trueDamage(target, 2, player, "§4" + target.getName() + " §7had their blood harvested by §4" + player.getName() + "§7's Nauseating Blood");
        AbilityUtils.addMaxHearts(player, 2);
        ...
        // Cancel any existing reset timer and start fresh (stacking resets the 30s window)
        UUID uuid = player.getUniqueId();
        org.bukkit.scheduler.BukkitTask old = harvestResetTask.remove(uuid);
        if (old != null) old.cancel();
        harvestResetTask.put(uuid, runLaterTask(() -> {
            harvestResetTask.remove(uuid);
            if (player.isOnline()) {
                AbilityUtils.resetMaxHearts(player);
                player.sendMessage("§c🩸 Blood Harvest faded. Hearts restored to 10.");
            }
        }, 600L)); // 30s
    }
    ...
    public void applyPassiveEffects(Player target) {
        // Blood Harvest: aggression and life-force
    ...
    public String[] getCooldownKeys() {
        return new String[]{"A1_NauseBlood", "A2_Sacrifice", "A3_Katana"};
    }

    @Override
    public boolean isAbilityActive(int slot, java.util.UUID uuid) {
        if (slot == 3) return katanaActive.contains(uuid);
        return false;
    }

    /** Scans the whole inventory for the katana by its unique PDC id and removes it,
     *  restoring the previously saved item in its place. Works regardless of which slot
     *  the player moved the katana to. */
    private void removeKatanaFromInventory(Player player, UUID uuid) {
        String id = katanaIds.remove(uuid);
        ItemStack restore = savedMainHand.remove(uuid);
        if (id == null) return;
        org.bukkit.NamespacedKey nk = new org.bukkit.NamespacedKey(plugin, "blood_katana_id");
        for (int i = 0; i < player.getInventory().getSize(); i++) {
            ItemStack item = player.getInventory().getItem(i);
            if (item == null || !item.hasItemMeta()) continue;
            String storedId = item.getItemMeta().getPersistentDataContainer()
                    .get(nk, org.bukkit.persistence.PersistentDataType.STRING);
            if (id.equals(storedId)) {
                player.getInventory().setItem(i,
                        (restore != null && restore.getType() != Material.AIR) ? restore : null);
                return;
            }
        }
        // Katana not found in inventory (already dropped/consumed) — nothing to do
    }
