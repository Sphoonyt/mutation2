...
public class BloodSoldierMutation extends Mutation {
    ...
    public void activateAbility2(Player player) {
        ...
        player.sendMessage("§c🩸 Soldier's Sacrifice! Set to 8 hearts for Strength II!");
        double healthBefore = player.getHealth();
        // Set player to exactly 8 hearts (16 HP)
        ...
        // Restore original health after 8s
        runLater(() -> {
            if (player.isOnline()) {
                player.setHealth(Math.min(player.getMaxHealth(), healthBefore));
                player.sendMessage("§c🩸 Your health has been restored!");
    ...
    public void activateAbility3(Player player) {
        ...
        // Cost: set health 1.5 hearts lower (real hearts, not damage)
        AbilityUtils.removeHearts(player, 1.5);
    ...
    private void triggerBloodHarvest(Player player, LivingEntity target) {
        ...
        AbilityUtils.addHearts(player, 2);
