...
public class BloodSoldierMutation extends Mutation {
    ...
    public void activateAbility3(Player player) {
        ...
        // Save whatever is currently in main hand so we can restore it
        ItemStack previous = player.getInventory().getItemInMainHand().clone();
        savedMainHand.put(uuid, previous);
        ...
        // Remove after 8s, restore original item
        runLater(() -> {
            if (player.isOnline()) {
                katanaActive.remove(uuid);
                ItemStack held = player.getInventory().getItemInMainHand();
                // Only swap back if the katana is still in hand (player may have manually swapped)
                if (held != null && held.hasItemMeta()
                    ...
                    if (restore != null && restore.getType() != Material.AIR) {
