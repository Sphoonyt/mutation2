...
public class BloodSoldierMutation extends Mutation {
    ...
    public void activateAbility3(Player player) {
        ...
        // If no sword found, use current main hand slot
        ...
        final int finalSwordSlot = swordSlot;
        ...
        // Sword attack speed (1.6 = diamond/iron sword speed)
        meta.addAttributeModifier(
        ...
        player.getInventory().setItem(finalSwordSlot, katana);
        player.getInventory().setHeldItemSlot(finalSwordSlot);
        ...
        runLater(() -> {
            if (player.isOnline()) {
                katanaActive.remove(uuid);
                ItemStack inSlot = player.getInventory().getItem(finalSwordSlot);
                if (inSlot != null && inSlot.hasItemMeta()
                        && "§c🩸 Blood Katana".equals(inSlot.getItemMeta().getDisplayName())) {
                    ItemStack restore = savedMainHand.remove(uuid);
                    player.getInventory().setItem(finalSwordSlot,
