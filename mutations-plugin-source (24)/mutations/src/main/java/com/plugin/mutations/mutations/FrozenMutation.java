...
public class FrozenMutation extends Mutation {
    ...
    // Encases player in an ice cage, heals 3 hearts over 3s,
    // gives Hunger 255 for 3s (Saturation amplifier 254)
    @Override
    public void activateAbility1(Player player) {
        ...
        // Particles: big snowflake burst + ring
        world.spawnParticle(Particle.SNOWFLAKE, player.getLocation().add(0, 1, 0), 100, 0.8, 1.2, 0.8, 0.15);
        AbilityUtils.spawnRing(player.getLocation(), 2.5, Particle.SNOWFLAKE, null, 36);
        world.spawnParticle(Particle.ITEM_SLIME, player.getLocation().add(0,1,0), 20, 0.5, 0.8, 0.5, 0.05);
        ...
        // Hunger 255 for 3s = Saturation amplifier 254 for 60 ticks
        player.addPotionEffect(new PotionEffect(PotionEffectType.SATURATION, 60, 254, false, false));
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            if (!player.isOnline() || healCount[0] >= 3) {
                ...
                world.spawnParticle(Particle.SNOWFLAKE, player.getLocation().add(0, 1, 0), 80, 0.6, 1, 0.6, 0.2);
                AbilityUtils.spawnRing(player.getLocation(), 2.5, Particle.SNOWFLAKE, null, 32);
                return;
            }
