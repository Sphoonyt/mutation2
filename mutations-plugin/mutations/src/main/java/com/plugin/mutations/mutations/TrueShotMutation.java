...
public class TrueShotMutation extends Mutation {
    ...
    public void onAssign(Player player) {
        ItemStack a1 = AbilityUtils.createAbilityItem(Material.ARROW,
                "§a[A1] Penetrating Volley", "3 rapid piercing arrows | 16s CD");
        ItemStack a2 = AbilityUtils.createAbilityItem(Material.SPECTRAL_ARROW,
                "§a[A2] Guided Light Arrow", "Homing arrow + mark | 18s CD");
        ItemStack a3 = AbilityUtils.createAbilityItem(Material.TRIPWIRE_HOOK,
                "§a[A3] True Shot Apex", "Perfect piercing arrow | 25s CD");
    ...
    public void activateAbility1(Player player) {
        ...
        for (int i = 0; i < 3; i++) {
            runLater(() -> {
                ...
                arrow.setVelocity(player.getEyeLocation().getDirection().normalize().multiply(3.9)); // +30% speed
                ...
                arrow.setMetadata("volley_hit_list", new FixedMetadataValue(plugin, "true"));
                totalHit.add(null); // Track for bonus check
    ...
    public void activateAbility2(Player player) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            if (nearest != null) {
                ...
                Vector vel = arrow.getVelocity().add(dir.multiply(0.15)).normalize().multiply(2.5);
