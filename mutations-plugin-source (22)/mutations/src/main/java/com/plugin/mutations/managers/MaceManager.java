...
public class MaceManager {
    ...
    /**
     * Call each copied mutation's applyPassiveEffects for the mace holder.
     * applyPassiveEffects() applies short-duration potion effects (30 ticks each),
     * refreshed every 5 ticks, representing that mutation's passive aura.
     * This is the correct hook — onTick() uses internal UUID maps keyed to the
     * source player and does nothing useful when called with the mace holder's player.
     */
    private void tickPassives(Player maceHolder) {
        ...
        for (Mutation mut : map.values()) {
            try {
                mut.applyPassiveEffects(maceHolder);
            } catch (Exception ignored) {}
        }
    }

    /** Returns true if the player has the Mace of Meiosis anywhere in their inventory */
