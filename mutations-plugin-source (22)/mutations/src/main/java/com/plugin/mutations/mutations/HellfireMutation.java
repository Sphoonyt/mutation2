...
public class HellfireMutation extends Mutation {
    ...
    private void executeWhipHit(Player player, int stage) {
        ...
        if (stage < 2) {
            ...
            for (LivingEntity e : hit) {
                AbilityUtils.trueDamage(e, 1.5, player); // 1.5 hearts AoE
                e.setFireTicks(40);
    ...
    public void activateAbility3(Player player) {
        ...
        AbilityUtils.spawnRing(player.getLocation().clone().add(0, 0.5, 0), 6, Particle.FLAME, null, 30);
        ...
        for (LivingEntity target : nearby) {
            ...
            plugin.getServer().getScheduler().runTaskTimer(plugin, hookTask -> {
                ...
                world.spawnParticle(Particle.FLAME, hookLoc, 4, 0.05, 0.05, 0.05, 0.005);
                world.spawnParticle(Particle.SMALL_FLAME, hookLoc, 2, 0.03, 0.03, 0.03, 0);
                ...
                if (tick[0] == totalTicks - 1) {
                    ...
                    AbilityUtils.trueDamage(finalTarget, 2.5, player); // 2.5 hearts
                    finalTarget.setFireTicks(80);
                    world.playSound(finalTarget.getLocation(), Sound.ENTITY_BLAZE_HURT, 1f, 0.8f);
                    world.spawnParticle(Particle.FLAME, finalTarget.getLocation().add(0,1,0), 20, 0.4, 0.4, 0.4, 0.08);
                }
