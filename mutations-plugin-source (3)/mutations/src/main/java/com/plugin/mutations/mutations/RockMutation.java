...
public class RockMutation extends Mutation {
    ...
    public void onAssign(Player player) {
        ItemStack a1 = AbilityUtils.createAbilityItem(Material.COBBLESTONE,
                "§6[A1] Stoneburst Slam", "5-block AOE, 4 dmg | 16s CD");
        ItemStack a2 = AbilityUtils.createAbilityItem(Material.STONE,
                "§6[A2] Skin Hardening", "Resistance II + Slowness 6s | 40s CD");
        ItemStack a3 = AbilityUtils.createAbilityItem(Material.GRAVEL,
                "§6[A3] Boulder Barrage", "3 boulders, 3 dmg each | 20s CD");
    ...
    public void activateAbility1(Player player) {
        ...
        world.spawnParticle(Particle.EXPLOSION, loc, 3, 0.5, 0.1, 0.5, 0.05);
    ...
    public void activateAbility3(Player player) {
        ...
        // Launch 3 boulders with staggered timing
        for (int i = 0; i < 3; i++) {
            final int index = i;
            runLater(() -> {
                if (!player.isOnline()) return;
                Vector dir = player.getEyeLocation().getDirection().normalize();
                // Slight spread for multiple boulders
