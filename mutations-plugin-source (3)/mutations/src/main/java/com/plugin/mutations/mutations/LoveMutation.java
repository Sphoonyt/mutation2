...
public class LoveMutation extends Mutation {
    ...
    public void onAssign(Player player) {
        ItemStack a1 = AbilityUtils.createAbilityItem(Material.PINK_DYE,
                "§d[A1] Heart Pulse Burst", "4-block shockwave + charm | 18s CD");
        ItemStack a2 = AbilityUtils.createAbilityItem(Material.MAGENTA_DYE,
                "§d[A2] Adoration Beam", "10-block beam 5 dmg | 16s CD");
        ItemStack a3 = AbilityUtils.createAbilityItem(Material.RED_DYE,
                "§d[A3] Love Overdrive", "+30% spd +20% dmg 10s | 45s CD");
    ...
    public void activateAbility2(Player player) {
        ...
        player.sendMessage("§d💖 Adoration Beam!");

        World world = player.getWorld();
        world.playSound(player.getLocation(), Sound.ENTITY_GUARDIAN_ATTACK, 1f, 0.5f);

        double baseDamage = 10; // 5 dmg
        boolean devActive = isDevotionActive(player);
        if (devActive) {
            baseDamage += 4; // +2 bonus = 7 dmg
            player.sendMessage("§d Devotion bonus! +2 extra damage");
        }

        final double dmg = baseDamage;
        Set<LivingEntity> hit = AbilityUtils.beamDamage(player, 10, 1.2,
                Particle.DUST, Color.fromRGB(255, 100, 200), dmg);

        for (LivingEntity e : hit) {
            world.spawnParticle(Particle.HEART, e.getLocation().add(0,1,0), 5, 0.3, 0.5, 0.3, 0.1);
        }
    }
    ...
    public void activateAbility3(Player player) {
        ...
        // Activation pulse
        List<LivingEntity> nearby = AbilityUtils.getNearby(player.getLocation(), 3, player);
        for (LivingEntity e : nearby) {
            e.damage(2, player);
        }

        // Speed (30% ≈ Speed I), extra hearts via absorption
        player.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 200, 1)); // Speed II ≈ 40%
        player.addPotionEffect(new PotionEffect(PotionEffectType.ABSORPTION, 200, 0)); // +2 hearts
        player.sendMessage("§d +30% Speed, +20% Damage, +2 Hearts for 10s!");
