...
public class DragonborneMutation extends Mutation {
    ...
    private void assignPoison(Player player) {
        ItemStack a1 = AbilityUtils.createAbilityItem(Material.DRAGON_BREATH,
                "§2[A1] Venom Burst", "AOE toxic smoke | 30s CD");
        ItemStack a2 = AbilityUtils.createAbilityItem(Material.SLIME_BALL,
                "§2[A2] Toxic Fang Strike", "Poison projectile | 14s CD");
        ItemStack a3 = AbilityUtils.createAbilityItem(Material.LIME_DYE,
                "§2[A3] Serpent Glide", "Dash + venom trail | 12s CD");
    ...
    private void assignFire(Player player) {
        ItemStack a1 = AbilityUtils.createAbilityItem(Material.FIRE_CHARGE,
                "§c[A1] Blazing Eruption", "Fiery AOE blast | 18s CD");
        ItemStack a2 = AbilityUtils.createAbilityItem(Material.BLAZE_POWDER,
                "§c[A2] Flame Talon Shot", "Explosive fire projectile | 16s CD");
        ItemStack a3 = AbilityUtils.createAbilityItem(Material.BLAZE_ROD,
                "§c[A3] Inferno Leap", "Fire leap | 12s CD");
        AbilityUtils.giveAbilityItems(player, a1, a2, a3);
    ...
    private void assignArmor(Player player) {
        ItemStack a1 = AbilityUtils.createAbilityItem(Material.IRON_INGOT,
                "§8[A1] Ironclad Surge", "Harden 5s, fragment burst | 35s CD");
        ItemStack a2 = AbilityUtils.createAbilityItem(Material.IRON_SWORD,
                "§8[A2] Steel Fist Barrage", "3-punch combo | 16s CD");
        ItemStack a3 = AbilityUtils.createAbilityItem(Material.ARMOR_STAND,
                "§8[A3] Armor Construct", "Revolving armor stand | 30s CD");
        AbilityUtils.giveAbilityItems(player, a1, a2, a3);
    ...
    private void venomBurst(Player player) {
        ...
        world.spawnParticle(Particle.SLIME, loc.add(0, 1, 0), 60, 2, 1, 2, 0.1);

        List<LivingEntity> nearby = AbilityUtils.getNearby(loc, 4, player);
        ...
        runLater(() -> {
            ...
            List<LivingEntity> finalHit = AbilityUtils.getNearby(player.getLocation(), 4, player);
            for (LivingEntity e : finalHit) {
                e.damage(10, player); // 5 hearts
                world.spawnParticle(Particle.SLIME, e.getLocation().add(0,1,0), 10, 0.3, 0.5, 0.3, 0.05);
            }
    ...
    private void toxicFangStrike(Player player) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            world.spawnParticle(Particle.SLIME, proj.getLocation(), 3, 0.1, 0.1, 0.1, 0);
        }, 0, 1);
    }
    ...
    private void serpentGlide(Player player) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            world.spawnParticle(Particle.SLIME, trailLoc, 5, 0.2, 0.1, 0.2, 0.02);

            // Damage and slow nearby entities in trail
            List<LivingEntity> hit = AbilityUtils.getNearby(trailLoc, 1.5, player);
            for (LivingEntity e : hit) {
                e.damage(1, player);
                e.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 20, 0, false, false));
            }

            if (player.isOnGround()) task.cancel();
        }, 0, 3);
    }
    ...
    private void spawnFireGround(Player player, Location center, int radius, long durationTicks) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            List<LivingEntity> hit = AbilityUtils.getNearby(center, radius, player);
    ...
    private void ironcladSurge(Player player) {
        ...
        runLater(() -> {
            ...
            List<LivingEntity> nearby = AbilityUtils.getNearby(player.getLocation(), 4, player);
            for (LivingEntity e : nearby) {
    ...
    private void steelFistBarrage(Player player) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            runLater(() -> {
                if (!player.isOnline()) return;
                List<LivingEntity> hit = AbilityUtils.getNearby(
                        player.getLocation().add(player.getLocation().getDirection().multiply(2)), 2.5, player);
                ...
                for (LivingEntity e : hit) {
                    if (punch < 2) {
                        ...
                        world.spawnParticle(Particle.EXPLOSION, e.getLocation(), 3, 0.3, 0.3, 0.3, 0.1);
                    }
                }
    ...
    private void armorConstruct(Player player) {
        ...
        plugin.getServer().getScheduler().runTaskTimer(plugin, task -> {
            ...
            List<LivingEntity> allies = AbilityUtils.getNearby(player.getLocation(), 3, player);
        ...
        runLater(() -> {
            ...
            List<LivingEntity> hit = AbilityUtils.getNearby(player.getLocation(), 3, player);
    ...
    public void onTick(Player player) {
        ...
        if (!wasOnGround && onGround) airJumpsUsed.put(uuid, 0);
        ...
        if (!wasOnGround && !onGround && prevY <= 0.05 && currY > 0.2) {
            ...
            if (jumps < MAX_AIR_JUMPS && (now - lastJump) >= AIR_JUMP_CD_MS) {
                ...
                if (style == Style.POISON) {
                    ...
                    world.spawnParticle(Particle.SLIME, player.getLocation(), 15, 0.5, 0.3, 0.5, 0.05);
                    List<LivingEntity> hit = AbilityUtils.getNearby(player.getLocation(), 3, player);
                    for (LivingEntity e : hit) {
                        e.damage(2, player);
