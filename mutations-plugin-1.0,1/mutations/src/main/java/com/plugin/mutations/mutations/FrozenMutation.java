...
public class FrozenMutation extends Mutation {
    ...
    private final Map<UUID, Map<Block, BlockData>> domainBlocks = new HashMap<>();
    ...
    public void onRemove(Player player) {
        ...
        Map<Block, BlockData> blocks = domainBlocks.remove(uuid);
    ...
    public void activateAbility1(Player player) {
        ...
        // Apply Hunger effect level 255 (amplifier 254) for 3s to everyone within 10 blocks (enemies & allies)
        List<LivingEntity> nearby = AbilityUtils.getNearbyTrusted(player.getLocation(), 10, player, plugin);
        for (LivingEntity target : nearby) {
            target.addPotionEffect(new PotionEffect(PotionEffectType.HUNGER, 60, 254, false, true));
            world.spawnParticle(Particle.SNOWFLAKE, target.getLocation().add(0, 1, 0), 30, 0.4, 0.6, 0.4, 0.08);
            if (target instanceof Player tp) {
                tp.sendMessage("§f❄️ " + player.getName() + " starved you with Frozen Recovery!");
            }
        }
        player.sendMessage("§f❄️ Starved " + nearby.size() + " target(s) nearby!");
    ...
    public void activateAbility2(Player player) {
        if (!checkCooldown(player, "A2_IceSpike", 28)) return;
        plugin.getAdvancementManager().onDamagingAbilityUsed(player, "frozen_a1");
            ...
            plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                ...
                plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                    ...
                    for (LivingEntity e : hit) {
                        AbilityUtils.trueDamage(e, 3, player, "§b" + e.getName() + " §7was impaled by §b" + player.getName() + "§7's Ice Spike");
                        e.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 80, 1));
                        // Knockback upward and away from spike
                        Vector kb = e.getLocation().toVector().subtract(groundLoc.toVector()).normalize();
                        kb.setY(0.5);
                        e.setVelocity(kb.multiply(1.2));
    ...
    public void activateAbility3(Player player) {
        ...
        int radius = 8;
        Map<Block, BlockData> placed = new LinkedHashMap<>();

        for (int x = -radius; x <= radius; x++) {
            for (int y = -1; y <= radius; y++) {
                for (int z = -radius; z <= radius; z++) {
                    double dist = Math.sqrt(x*x + y*y + z*z);
                    // Dome shell — replace whatever is there and save original state
                    if (dist >= radius - 0.9 && dist <= radius + 0.1 && y >= -1) {
                        Block b = center.clone().add(x, y, z).getBlock();
                        if (!PROTECTED_BLOCKS.contains(b.getLocation()) && b.getType() != Material.BEDROCK) {
                            placed.put(b, b.getBlockData().clone());
                            b.setType(Material.BLUE_ICE);
                            PROTECTED_BLOCKS.add(b.getLocation());
                        }
                    }
                    // Freeze ground (y=-1 level, below feet) inside the dome
                    if (y == 0 && dist <= radius - 1) {
                        ...
                        if (ground.getType().isSolid() && !ICE_TYPES.contains(ground.getType())
                                && !PROTECTED_BLOCKS.contains(ground.getLocation())
                                && ground.getType() != Material.BEDROCK) {
                            placed.put(ground, ground.getBlockData().clone());
                            ground.setType(Material.BLUE_ICE);
                            PROTECTED_BLOCKS.add(ground.getLocation());
                        }
                    }
                }
            }
    ...
    private void restoreBlocks(Map<Block, BlockData> blocks) {
