...
public class AbilityUtils {
    ...
    public static void trueDamage(LivingEntity entity, double hearts, Player source, String deathMessage) {
        ...
        if (newHp <= 0.0) {
            // Register kill credit — set last damage cause so getKiller() works
            entity.setLastDamageCause(new EntityDamageByEntityEvent(
                    source, entity, EntityDamageEvent.DamageCause.CUSTOM, dmg));

            if (entity instanceof Player player && hasTotem(player)) {
                entity.setHealth(1.0);
                entity.damage(1.0, source);
            } else {
                // Store custom death message before killing
